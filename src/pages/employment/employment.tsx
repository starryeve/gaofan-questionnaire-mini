...
function Employee() {
  ...
  const getQuestionnaires = async () => {
    ...
    const q = res.data.data.filter((item) => item.intern === 0)
    setQuestionnaires(q)
    console.log(res);
  }
  ...
  const addQuestionnaire = async () => {
    Taro.navigateTo({
      url: '/pages/newQuestionnaire/newQuestionnaire?intern=0'
    })
  }

  const handleModify = async (id) => {
    Taro.navigateTo({
      url: `/pages/modifyQuestionnaire/modifyQuestionnaire?intern=0&id=${id}`,
    })
  }
  ...
  return (
    <View className='employee'>
      ...
      <View className='title'>用人单位招聘岗位信息填报</View>
